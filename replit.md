...
- July 8, 2025: Sistema administrativo expandido e responsividade móvel corrigida - Implementados modais "Adicionar Contrato" e "Adicionar Categoria" na seção administrativa do sidebar; Função de scroll adicionada ao menu lateral; Corrigido problema crítico de navegação móvel que causava telas brancas (implementado timeout na navegação e melhorado controle de cache React Query); Hook useResponsive otimizado com throttling e inicialização SSR; Sidebar com prevenção de scroll do body em mobile; Endpoints /api/contracts e /api/categories criados para administração; Sistema totalmente funcional em todos os dispositivos
