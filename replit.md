...
- July 3, 2025: Conexão com banco Supabase corrigida e sistema de constantes padronizadas implementado - Migração completa do driver @neondatabase/serverless para pg (node-postgres) para compatibilidade com Supabase; Configuração centralizada de banco em server/database.ts; Coluna bank_issuer adicionada ao banco via SQL; Sistema de 15 contratos, 43 categorias, 3 bancos e 5 formas de pagamento padronizadas em shared/constants.ts; Funções de normalização inteligente para importação Excel utilizando constantes; Todos os dropdowns e filtros atualizados para usar listas consistentes; Sistema 100% funcional com dados reais do Supabase
