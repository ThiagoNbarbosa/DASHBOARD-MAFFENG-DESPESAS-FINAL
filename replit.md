...
- July 1, 2025: Sistema de padrões e validação rigorosa implementado - Criados padrões fixos para contratos (CONT-0001 a CONT-0005, CONT-MAFF-001 a 003), categorias (Alimentação, Transporte, Material, Serviços, etc.) e bancos (principais bancos brasileiros); Adicionado campo 'banco pagador' em despesas; Novo endpoint /api/expenses/import-excel-strict com validação rigorosa que bloqueia importação se dados divergem dos padrões; Sistema de drag and drop funcional no modal de importação; Endpoint /api/standards para fornecer padrões ao frontend; Preparação para filtros por período de datas (startDate/endDate)
