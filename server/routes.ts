...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Novo endpoint de importação Excel com validação rigorosa
  app.post("/api/expenses/import-excel-strict", requireAuth, upload.single('excel'), async (req, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ message: "Nenhum arquivo fornecido" });
      }

      console.log('Iniciando importação com validação rigorosa...');
      ...
      // Primeira passada: validar todos os dados antes de importar
      const rows = data.slice(1);
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (!row || row.length < 7) {
          validationErrors.push(`Linha ${i + 2}: dados insuficientes (esperado: item, valor, método pagamento, categoria, contrato, data, banco)`);
          continue;
        }

        const [rawItem, rawValue, rawPaymentMethod, rawCategory, rawContract, rawDate, rawBank] = row;
        
        if (!rawItem || !rawValue) {
          validationErrors.push(`Linha ${i + 2}: item ou valor obrigatório em branco`);
          continue;
        }

        // Validação rigorosa de categoria
        const normalizedCategory = normalizeCategory(rawCategory);
        if (!normalizedCategory) {
          validationErrors.push(`Linha ${i + 2}: categoria "${rawCategory}" não está nos padrões permitidos. Categorias válidas: ${STANDARD_CATEGORIES.join(', ')}`);
          continue;
        }

        // Validação rigorosa de contrato
        const normalizedContract = normalizeContract(rawContract);
        if (!normalizedContract) {
          validationErrors.push(`Linha ${i + 2}: contrato "${rawContract}" não está nos padrões permitidos. Contratos válidos: ${STANDARD_CONTRACTS.join(', ')}`);
          continue;
        }

        // Validação rigorosa de banco
        const normalizedBank = normalizeBank(rawBank);
        if (!normalizedBank) {
          validationErrors.push(`Linha ${i + 2}: banco "${rawBank}" não está nos padrões permitidos. Bancos válidos: ${STANDARD_BANKS.join(', ')}`);
          continue;
        }
        ...
        if (typeof rawValue === 'number') {
          ...
          value = parseFloat(cleanValue);
          if (isNaN(value)) {
            validationErrors.push(`Linha ${i + 2}: valor inválido "${rawValue}"`);
            continue;
          }
        }
        ...
        if (typeof rawDate === 'number') {
          ...
          if (isNaN(paymentDate.getTime())) {
            validationErrors.push(`Linha ${i + 2}: data inválida "${rawDate}"`);
            continue;
          }
        } else {
          validationErrors.push(`Linha ${i + 2}: data é obrigatória`);
          continue;
        }
        ...
        validatedRows.push({
          ...
          value: value.toString(),
          totalValue: value.toString(),
          paymentMethod: String(rawPaymentMethod).trim(),
          category: normalizedCategory,
          contractNumber: normalizedContract,
          paymentDate,
          paymentBank: normalizedBank,
          imageUrl: '',
        });
      }

      // Se há erros de validação, retornar sem importar nada
      if (validationErrors.length > 0) {
        return res.status(400).json({
          message: "Importação cancelada devido a dados divergentes dos padrões",
      ...
      // Segunda passada: importar dados validados
      let imported = 0;
      for (const expenseData of validatedRows) {
        try {
          await storage.createExpense({
            ...expenseData,
            userId: req.session.userId!,
      ...
      res.json({
        ...
        message: `${imported} despesas importadas com sucesso. Todos os dados estão em conformidade com os padrões.`
      });
